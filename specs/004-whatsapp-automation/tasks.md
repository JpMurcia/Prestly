...
- [X] **T005 revisado post-implementación** (no era una tarea planeada — hallazgo al auditar la migración contra el skill `supabase-postgres-best-practices`, que debió cargarse antes de escribirla y no se hizo): la versión original comprobaba `NOT EXISTS` en `notificaciones_whatsapp` y recién insertaba al final — dos corridas concurrentes podían pasar ambas la comprobación antes de que cualquiera insertara y **las dos** habrían llamado a Twilio de verdad para la misma cuota (la fila duplicada se evitaba, el envío real no). Corregido: `pg_try_advisory_xact_lock` al inicio + reclamo atómico (`INSERT ... ON CONFLICT DO NOTHING RETURNING id`, estado provisional `'simulado'`) **antes** de tocar `net.http_post`, actualizando esa misma fila después con el resultado real. Ver research.md §8. Re-verificado tras `db reset`: mismo comportamiento en modo simulado (paso 2-3 de arriba) más un nuevo caso — dos corridas de `revisar_y_enviar_notificaciones_whatsapp()` disparadas al mismo tiempo contra la misma cuota recién elegible → exactamente una la procesa, la otra devuelve 0 filas, una sola fila final en la tabla (quickstart.md paso 3b).
