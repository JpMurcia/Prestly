...
3b. **Concurrencia** (research.md §8): con una cuota recién elegible, disparar dos corridas de `revisar_y_enviar_notificaciones_whatsapp()` al mismo tiempo (dos `docker exec ... psql` en paralelo) → exactamente una de las dos procesa la cuota (la otra devuelve 0 filas), y queda una sola fila en `notificaciones_whatsapp` — nunca dos.
