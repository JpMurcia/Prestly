...
import { Button, Card, Chip } from '@repo/ui/native';
...
export function QuoteCalculatorScreen() {
  ...
  function handleShare() {
    ...
    // Share nativo del dispositivo — acción manual (FR-003), no notificación automatizada.
  ...
  return (
    <ScrollView className="flex-1 bg-neutral-50" contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <Pressable testID="quote-toggle-table" onPress={() => setShowTable((v) => !v)}>
        <Text className="text-center font-semibold text-brand-navy">
          {showTable ? 'Ver resumen' : 'Ver tabla completa'}
        </Text>
      </Pressable>
      ...
      <Button testID="quote-share" label="Compartir tabla" variant="secondary" onPress={handleShare} disabled={!canIssue} />
